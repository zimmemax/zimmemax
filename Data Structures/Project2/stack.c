 ...
 *
 * Name: Max Zimmer
 * Email: zimmemax@oregonstate.edu
 */
...
//while stack is not empty pop elements... use is empty
struct node* nodePtr = get_head(stack->list);
...
void* stack_top(struct stack* stack) {
	return get_node_val(get_head(stack->list));
...
void* stack_pop(struct stack* stack) {
	...
	struct node* temp = get_head(stack->list);
	temp = list_remove(stack->list, get_node_val(temp), compare);
	return temp;
